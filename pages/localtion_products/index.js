...
Page({
  ...
  bindProductshowWay: function () {
    ...
    } else if (this.data.ProductshowWay == 2) {
      this.setData({ ProductshowWay: 0 })
      let that = this;
      wx.getLocation({
        type: 'gcj02', //返回可以用于wx.openLocation的经纬度
        success: function (res) {
          console.log(res)
          that.data.local.latitude = res.latitude
          that.data.local.longitude = res.longitude
          that.setData({
